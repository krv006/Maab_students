...
def normalize_emails(value):
    ...
    if isinstance(value, list):
        for item in value:
            if isinstance(item, dict):
                ...
                if isinstance(email, str) and email:
                    ...
                    result.extend([e.lower().strip() for e in email if isinstance(e, str) and e])
            elif isinstance(item, str) and item:
                try:
                    parsed = ast.literal_eval(item)
                    if isinstance(parsed, list):
                        result.extend([e.lower().strip() for e in parsed if isinstance(e, str) and e])
                    elif isinstance(parsed, str):
                        result.append(parsed.lower().strip())
                except Exception:
                    result.extend([e.lower().strip() for e in item.split(",") if e.strip()])
        ...
        try:
            ...
            result.extend([e.lower().strip() for e in value.split(",") if e.strip()])
    return result


# --- Load Data ---
try:
    df_raw = pd.read_csv("odilbek.csv")
except FileNotFoundError:
    print("Xato: 'odilbek.csv' fayli topilmadi. Fayl yo'lini tekshiring.")
    exit()
...
# --- Check if comm_type exists ---
if "comm_type" not in df.columns:
    print(f"Xato: 'comm_type' ustuni mavjud emas. Mavjud ustunlar: {df.columns.tolist()}")
    exit()
...
# --- Debugging: Print sample data to verify recipients ---
...
# --- Export ---
with pd.ExcelWriter("star_schema_odilbek1.xlsx", engine="xlsxwriter") as writer:
