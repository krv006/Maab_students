...
# Merge user_id from dim_user_full
email_df = email_df.merge(
    dim_user_full[["name", "email", "user_id"]],
    on=["name", "email"],
    how="left"
)

# Assign new user_id if still missing
...
# --- Export ---
with pd.ExcelWriter("star_schema_full_dimensions_final_v3.xlsx", engine="xlsxwriter") as writer:
